...
const Home = () => {
  ...
  return (
    <View style={{marginTop: 50, alignContent: 'center', alignItems:'center'}}>
      <Text style={{fontSize: 30, fontWeight:'bold'}}>React Native Boilerplate</Text>
      <View style={{marginTop: 40,  alignContent: 'center', alignItems:'center'}}>
        <Text >Test Action Status: {status ? 'ON' : 'OFF'}</Text>
        {status ?
        <View style={{height:50,width:50, backgroundColor:'#53ab30', borderRadius: 50, marginTop: 30}}></View>
        :
        <View style={{height:50,width:50, backgroundColor:'#bf0000', borderRadius: 50, marginTop: 30}}></View>
        }
        <Button 
        mode="contained" 
        color='#008f8a'
        ...
        <View style={{padding:20, marginTop: 30, backgroundColor:'#efefef',  borderRadius: 20}}>
          {data && data.map((a, i)=>(
            <View key={i}>
